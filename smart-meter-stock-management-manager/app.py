# ====================================================
# === app.py : eThekwini Smart Meter Stock Management ===
# ====================================================
...
for d in [DATA_DIR, DUMP_DIR, PHOTO_DIR, ISSUED_PHOTOS_DIR, REPORT_DIR]:
...
def save_data(df):
    ...
    # Create a daily dump
    dump_path = DUMP_DIR / f"stock_requests_{datetime.now().strftime('%Y-%m-%d')}.csv"
    shutil.copy(DATA_FILE, dump_path)
...
def city_ui():
    ...
    if sel:
        ...
        qty = st.number_input("Approved Qty", 0, value=int(record["Requested_Qty"]))
        photo = st.file_uploader("Upload proof photo", type=["jpg", "png"])
        notes = st.text_area("City Notes")
...
def manager_ui():
    ...
    if dumps:
        dump_file = st.selectbox("Select Dump File", dumps)
        dump_df = pd.read_csv(dump_file)
        st.dataframe(dump_df, use_container_width=True)
        st.download_button("⬇️ Download Selected Dump", data=dump_df.to_csv(index=False), file_name=dump_file.name)
    else:
