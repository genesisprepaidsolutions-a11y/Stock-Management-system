...
st.set_page_config(page_title="Stock Request Workflow", page_icon="📦", layout="wide")
...
def load_data():
    if DATA_FILE.exists():
        try:
            ...
            pass
    cols = ["Date_Requested","Request_ID","Contractor_Name","Installer_Name","Meter_Type","Requested_Qty",
            "Approved_Qty","Photo_Path","Status","Contractor_Notes","City_Notes","Decline_Reason","Date_Approved","Date_Received"]
    return pd.DataFrame(columns=cols)

def save_data(df): df.to_csv(DATA_FILE, index=False)

def generate_request_id(): return f"REQ-{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
def contractor_ui():
    ...
    meter_type = st.selectbox("Meter Type", ["DN15 Meter", "CIU Keypad"])
    qty = st.number_input("Requested Quantity", 1)
    notes = st.text_area("Notes")
    if st.button("Submit Request"):
        if not installer_name:
            st.warning("Enter installer name")
        else:
            df = load_data()
            rid = generate_request_id()
            entry = {"Date_Requested":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                     "Request_ID":rid,"Contractor_Name":contractor_name,"Installer_Name":installer_name,
                     "Meter_Type":meter_type,"Requested_Qty":qty,"Approved_Qty":"","Photo_Path":"",
                     "Status":"Pending Verification","Contractor_Notes":notes,"City_Notes":"","Decline_Reason":"",
...
def city_ui():
    st.header("City - Verify Requests")
    df = load_data()
...
def installer_ui():
    st.header("Installer - Receive Stock")
    df = load_data()
    installer = st.session_state.auth["name"]
    approved = df[(df["Installer_Name"]==installer)&(df["Status"].str.contains("Approved"))]
    st.dataframe(approved)
    sel = st.selectbox("Mark as received ID", [""]+approved["Request_ID"].tolist())
...
def manager_ui():
    ...
    approved=(df["Status"].str.contains("Approved")).sum()
...
if not st.session_state.auth["logged_in"]:
    ...
    if role=="contractor": contractor_ui()
    elif role=="city": city_ui()
    elif role=="installer": installer_ui()
    elif role=="manager": manager_ui()
